    ...
    <div className="animate-expand">
      ...
      {(totals.spending > 0 || totals.income > 0) && (
        <div className="flex gap-4 mt-3 pb-3 border-b border-line text-sm">
          {totals.spending > 0 && <span className="font-semibold text-peach-600">−{money(totals.spending)}</span>}
    ...
    <div className="animate-expand">
      ...
      <div className="mt-3 space-y-2.5">
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="font-semibold text-peach-600">−{money(summary.spending)}</span>
        </div>
        <div className="flex items-center justify-between text-sm pt-2.5 border-t border-line">
          <span className="text-ink-soft">Net</span>
          <span className={`font-semibold ${net >= 0 ? 'text-sage-600' : 'text-peach-600'}`}>
            {net >= 0 ? '+' : '−'}{money(net)}
          </span>
        </div>
      </div>
            ...
            >
              ...
              {hasData && (
                <span className="mt-auto flex flex-col items-center leading-tight">
                  {spending > 0 && (
                    <span className="text-[10px] sm:text-[11px] font-medium text-peach-600">−{money0(spending)}</span>
                  )}
                  {income > 0 && (
...
// Each day is a heat-tinted cell — sage for net-positive days, peach for
    ...
    <div>
      ...
      <div className="grid grid-cols-7 gap-[3px]">
        {cells.map((day, i) => {
          ...
          else if (has && net < 0) bg = `rgba(209,138,91,${0.25 + mag * 0.6})`
          const isSelected = selectedDate === key
          return (
            <button
              key={key}
