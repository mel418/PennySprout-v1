...
//   plaid-link / plaid-sync — Pro-only routes (see lib/planGate.js
//     requirePro, checked before the rate limit) — the free entries below
//     are normally unreachable, but they must stay non-zero: checkRateLimit
//     treats a falsy limit as "no limit configured" (see the `if (!limit)`
//     check below), so a 0 here would mean UNLIMITED, not blocked.
...
const DAILY_LIMITS = {
  free: { 'chat': 30, 'parse-pdf': 20, 'plaid-link': 3, 'plaid-sync': 3, 'write': 500 },
  pro:  { 'chat': 200, 'parse-pdf': 100, 'plaid-link': 10, 'plaid-sync': 20, 'write': 500 },
}
