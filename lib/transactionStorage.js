...
// legacy key casing, no full account resolution) since this isn't feeding
// any of the calc/chart helpers that expect the client shape. Paginated the
// same way getTransactions is — an unbounded select silently truncates at
...
export async function getHiddenTransactions(userId) {
  ...
  for (let page = 0; ; page++) {
    const { data, error } = await supabase
      .from('transactions')
      .select('id, file_id, date, description, amount, category, source, hidden_reason, hidden_at')
      .eq('user_id', userId)
      .eq('hidden', true)
  ...
  // Resolve each row's source file name so the panel can group hidden
  // transactions by statement and offer "Restore all from this file". Most
  // hidden rows are uploads and have a file_id, but sync's auto-hide (and
  // the duplicate scan) can now also hide an orphaned Plaid row left behind
  // by a disconnect-then-reconnect of the same bank (see
  // findDuplicateCandidates) — those have no file_id, so fileName falls
  ...
  if (fileIds.length > 0) {
    ...
    if (fileError) {
      console.error('Error fetching file names for hidden transactions:', fileError)
    } else {
      fileNameById = Object.fromEntries(files.map(f => [f.id, f.file_name]))
    }
  }

  return rows.map(row => ({ ...row, fileName: fileNameById[row.file_id] || null }))
}
...
// data.
//
// Also matches against orphaned Plaid rows (source = 'plaid', plaid_item_id
// IS NULL) — transactions left behind by a disconnect that didn't delete
// history (see app/api/plaid/items/[id]/route.js). Reconnecting the same
// institution gives Plaid a new Item with entirely new
// plaid_transaction_ids for the same real-world transactions, so the
// transactions_user_plaid_txn_key unique constraint never catches the
// re-sync; without this, every reconnect duplicated the connection's whole
// history.
export async function findDuplicateCandidates(userId, plaidRows) {
  ...
  const { data, error } = await supabase
    ...
    .eq('user_id', userId)
    .or('source.eq.upload,and(source.eq.plaid,plaid_item_id.is.null)')
    .eq('hidden', false)
    .gte('date', fromDate)
    .lte('date', toDate)
