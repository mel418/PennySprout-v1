...
export async function getHiddenTransactions(userId) {
  ...
  // transactions by statement and offer "Restore all from this file" — every
  // hidden row today came from an upload (see setTransactionsHidden's
  // callers: sync's auto-hide and the duplicate scan both only ever hide the
  // upload side of a match, and hideTransactionsByFile only ever touches
  // upload rows), so file_id is set on all of them in practice, but
  // fileName still falls back to null for a row where it isn't.
