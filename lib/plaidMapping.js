...
// Plaid's transaction_code is a MUCH better signal than the text patterns
// above when it's populated — it's a structured field, not a regex guess —
// but it's "populated for select financial institutions" and null for
// others (confirmed against two real connections: always present and
// accurate for one card issuer, always null for the credit union). So it's
// checked first, with the text patterns as a fallback for institutions that
// don't set it.
//
// 'payment' caught a real bug: a credit card's bill payment (paying the
// card off from a checking/savings account) landed on TRANSFER_IN with a
// LOW-confidence detailed value, got promoted to 'Income' by the same
// positive-amount rule that handles Zelle-received — a payment TOWARD a
// bill isn't income, it's the bill itself. 'bill payment', 'direct debit',
// and 'standing order' are Plaid's own more specific bill-shaped codes, and
// a transaction Plaid tags with any of them should never have needed the
// ambiguous-Transfer fallback logic below in the first place.
const BILL_PAYMENT_CODES = new Set(['payment', 'bill payment', 'direct debit', 'standing order'])

// transaction_code 'adjustment' ("Bank adjustment") isn't specific to the
// ambiguous Transfer bucket — verified against a real connection, it showed
// up on BOTH a GENERAL_MERCHANDISE-categorized clothing return credit and an
// OTHER-categorized cash-back reward credit. Both are money coming back to
// the user, not new spending and not really new income either — like a
// refund, just under a PFC primary that could be anything. So this is
// checked before category resolution, but ONLY when the transaction is a
// credit (Plaid's amount is negative — money in): a DEBIT 'adjustment' (e.g.
// a fee correction taking money out) is left under whatever category it
// already resolved to, since that's still genuinely money leaving.
function isAdjustmentCredit(transactionCode, plaidAmount) {
  return transactionCode === 'adjustment' && typeof plaidAmount === 'number' && plaidAmount < 0
}

function inferAmbiguousTransferCategory(description, transactionCode) {
  if (transactionCode === 'refund') return 'Refund'
  if (BILL_PAYMENT_CODES.has(transactionCode)) return 'Bills'
  if (INTERNAL_TRANSFER_TEXT.test(description)) return 'Account Transfer'
  if (CARD_REFUND_TEXT.test(description)) return 'Refund'
  return null
}

export function mapPfcToCategory(pfc, description = '', transactionCode = null, plaidAmount = null) {
  if (pfc?.detailed && INTERNAL_TRANSFER_DETAILS.has(pfc.detailed)) return 'Account Transfer'
  if (isAdjustmentCredit(transactionCode, plaidAmount)) return 'Refund'
  const primary = pfc?.primary
  const base = primary ? (PFC_TO_CATEGORY[primary] || 'Other') : 'Other'
  if (base === 'Transfer') {
    const inferred = inferAmbiguousTransferCategory(description, transactionCode)
...
export function plaidTxnToRow(txn, { userId, itemRowId }) {
  ...
  return {
    ...
    category: mapPfcToCategory(txn.personal_finance_category, rawDescription, txn.transaction_code, txn.amount),
