...
// and Transfer (moving money between accounts isn't a purchase).
export function calcSpending(transactions) {
  return transactions.reduce((sum, t) => {
    const cat = normalizeCategory(t.Category, t.Amount)
    if (cat === 'Income' || cat === 'Bills & Payments' || cat === 'Transfer') return sum
...
// Per-category discretionary totals, largest first (excludes Income, Bills & Transfer).
