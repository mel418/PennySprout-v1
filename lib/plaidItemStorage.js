...
// Maps Plaid's /accounts/get response accounts into the shape
// upsertPlaidAccounts expects — shared by the initial Link exchange (first
// balance snapshot) and every subsequent sync (balance refresh; see
// lib/plaidSyncEngine.js), so the two call sites can't quietly drift on
// which fields get pulled off Plaid's account object.
export function plaidAccountsToRows(accounts) {
...
export async function getPlaidItems(userId) {
  ...
  const { data: accounts, error: accError } = await supabase
    .from('plaid_accounts')
    .select('id, item_id, account_id, name, official_name, mask, type, subtype, current_balance, available_balance, iso_currency_code, balance_updated_at')
    .eq('user_id', userId)
