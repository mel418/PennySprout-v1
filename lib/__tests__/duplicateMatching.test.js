...
describe('matchDuplicateCandidates', () => {
  ...
  it('a same-amount pair further apart than the default (real case: a card-issuer migration artifact, not a normal offset) is NOT caught automatically', () => {
    // This 5-day gap was a one-off from migrating cards mid-month, not the
    // pattern the default is sized for (see lib/duplicateMatching.js) — a
    // case like this is still findable by widening toleranceDays for a
    // specific scan (see the next test), just not automatically.
    ...
    expect(matchDuplicateCandidates(plaidRows, uploadRows)).toEqual([])
    expect(matchDuplicateCandidates(plaidRows, uploadRows, { toleranceDays: 5 })).toHaveLength(1)
  })
  ...
  it('the tolerance is configurable per call, not just the default', () => {
    const plaidRows = [{ plaid_transaction_id: 'p1', date: '2026-07-01', amount: -20, description: 'Target' }]
    const uploadRows = [{ id: 'u1', date: '2026-07-04', amount: -20, description: 'Target' }] // 3 days out
    expect(matchDuplicateCandidates(plaidRows, uploadRows)).toEqual([]) // default (2 days) misses it
    expect(matchDuplicateCandidates(plaidRows, uploadRows, { toleranceDays: 3 })).toHaveLength(1)
  })
