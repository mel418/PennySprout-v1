...
describe('categoryColor', () => {
  ...
  it('uses the semantic colors for the well-known categories', () => {
    expect(categoryColor('Income')).toBe(SEMANTIC_COLORS.income)
    expect(categoryColor('Bills & Payments')).toBe(SEMANTIC_COLORS.bills)
    expect(categoryColor('Subscriptions')).toBe(SEMANTIC_COLORS.subscriptions)
    expect(categoryColor(null)).toBe(SEMANTIC_COLORS.neutral) // falls back to Other
  })
})

describe('categoryTint', () => {
  it('returns the category color as rgba at the requested alpha', () => {
    expect(categoryTint('Income', 0.2)).toBe('rgba(124, 154, 116, 0.2)')
  })
})
...
describe('calcSpending', () => {
  it('sums absolute spending, excluding Income, Bills & Payments, and Transfer', () => {
    // 4.50 coffee + 30 groceries = 34.50 (Zelle sent isn't discretionary spending)
    expect(calcSpending(TXNS)).toBeCloseTo(34.5)
  })
  ...
  it('excludes Account Transfer regardless of sign (moving money between your own accounts, e.g. Plaid savings sweeps)', () => {
    expect(calcSpending([{ Amount: '-200', Category: 'Account Transfer' }])).toBe(0)
    expect(calcSpending([{ Amount: '200', Category: 'Account Transfer' }])).toBe(0)
  })

  it('excludes Refund regardless of sign (a merchant refund isn\'t new spending, and isn\'t income either)', () => {
    expect(calcSpending([{ Amount: '25', Category: 'Refund' }])).toBe(0)
    expect(calcSpending([{ Amount: '-25', Category: 'Refund' }])).toBe(0)
  })
})
...
describe('calcIncome', () => {
  ...
  it('never counts a positive Account Transfer as income', () => {
    expect(calcIncome([{ Amount: '536', Category: 'Account Transfer' }])).toBe(0)
  })
})
