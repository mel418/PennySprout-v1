...
describe('parseDate', () => {
  ...
  it('parses ISO date-only strings as LOCAL dates (no UTC day shift)', () => {
    // The transactions table stores dates as 'YYYY-MM-DD'. new Date() would
    // treat that as UTC midnight — the previous day in US timezones.
    const d = parseDate({ 'Date': '2026-07-04' })
    expect(toKey(d)).toBe('2026-07-04')
  })
