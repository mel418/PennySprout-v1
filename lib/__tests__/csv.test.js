...
import { parseCsvRows, parseTransactionsCsv, isAllowedHeader, serializeCsv, escapeCsvField } from '../csv'
...
describe('parseTransactionsCsv', () => {
  ...
  it('collapses split Debit/Credit columns into a signed Amount (Capital One-style)', () => {
    const csv = [
      'Transaction Date,Posted Date,Card No.,Description,Category,Debit,Credit',
      '2026-07-28,2026-07-28,9156,CAPITAL ONE MOBILE PYMT,Payment/Credit,,455.14',
      '2026-07-24,2026-07-27,9156,ZAGG - CENTURY CITY,Merchandise,12.00,',
    ].join('\n')
    const { headers, data } = parseTransactionsCsv(csv)
    expect(headers).toContain('Amount')
    expect(data[0]).toMatchObject({ Description: 'CAPITAL ONE MOBILE PYMT', Amount: '455.14' })
    expect(data[1]).toMatchObject({ Description: 'ZAGG - CENTURY CITY', Amount: '-12' })
  })

  it('leaves Amount alone when the file already has a single Amount column', () => {
    const csv = [
      'Date,Description,Amount,Debit',
      '01/01/2026,COFFEE,-4.50,',
    ].join('\n')
    const { data } = parseTransactionsCsv(csv)
    expect(data[0].Amount).toBe('-4.50')
  })
})
...
describe('serializeCsv', () => {
  it('round-trips through parseCsvRows', () => {
    const rows = [
      ...
      ['2026-07-01', 'AMAZON.COM, SEATTLE WA', '-42.10'],
      ['2026-07-02', 'Note with "quotes" inside', '12.00'],
      ['2026-07-03', 'Line\nbreak', '0.99'],
    ]
  ...
  it('quotes only fields that need it', () => {
    expect(serializeCsv([['plain', 'a,b', 'q"q']])).toBe('plain,"a,b","q""q"\r\n')
  })

  it('serializes null/undefined as empty fields', () => {
    expect(serializeCsv([['x', null, undefined]])).toBe('x,,\r\n')
  })
})
