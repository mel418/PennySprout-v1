...
export function parseTransactionsCsv(text) {
  ...
  // Some cards (e.g. Capital One) split the amount into separate Debit/Credit
  // columns instead of one signed Amount column. Collapse them into the
  // signed 'Amount' every downstream calc expects — debit is spending
  // (negative), credit is a payment/refund (positive).
  const amountHeader = headers.find(h => /^(transaction\s*)?amount$/i.test(h))
  const debitHeader = headers.find(h => /^debit$/i.test(h))
  const creditHeader = headers.find(h => /^credit$/i.test(h))
  if (!amountHeader && (debitHeader || creditHeader)) {
    data.forEach(row => {
      const debit = parseFloat(row[debitHeader]) || 0
      const credit = parseFloat(row[creditHeader]) || 0
      row['Amount'] = String(credit - debit)
    })
    headers.push('Amount')
  }
