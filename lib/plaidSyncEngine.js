...
import { findDuplicateCandidates } from '@/lib/transactionStorage'
...
export async function syncItem(userId, item, { email } = {}) {
  ...
  try {
    ...
    // Newly-added transactions may be the same real-world event as
    // something already manually uploaded (see lib/transactionStorage.js
    // findDuplicateCandidates) — surface candidates for the client to offer
    // a review, never hide anything automatically here.
    ...
    return { ok: true, ...totals, duplicateCandidates }
  } catch (error) {
    const code = plaidErrorCode(error)
