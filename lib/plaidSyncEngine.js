...
import { plaidTxnToClientShape, plaidTxnToRow } from './plaidMapping'
import { supabase } from '@/lib/supabase'
import { plaid, plaidErrorCode, isReauthRequired } from '@/lib/plaid'
import { updateItemCursor, markItemStatus, getUserItemsForSync, upsertPlaidAccounts, plaidAccountsToRows } from '@/lib/plaidItemStorage'
import { checkBudgetAlerts } from '@/lib/budgetAlerts'
import { findDuplicateCandidates, setTransactionsHidden } from '@/lib/transactionStorage'
...
export async function syncItem(userId, item, { email } = {}) {
  ...
  try {
    ...
    // Best-effort balance refresh, piggybacked on the sync that's already
    // talking to this item rather than a separate call site — shown next to
    // each account in ConnectedAccounts.js. A failure here must not fail the
    // sync itself: the transaction data above already applied fine, only the
    // balance display goes briefly stale.
    try {
      const { data: accountsData } = await plaid.accountsGet({ access_token: item.accessToken })
      await upsertPlaidAccounts(userId, item.id, plaidAccountsToRows(accountsData.accounts))
    } catch (error) {
      console.error('Error refreshing account balances after sync:', plaidErrorCode(error) || error)
    }
    ...
    // Newly-added Plaid transactions may be the same real-world event as
    // something already manually uploaded (see lib/transactionStorage.js
    // findDuplicateCandidates) — hidden automatically here rather than left
    // for the user to find and clear by hand. matchDuplicateCandidates only
    // matches on amount + a narrow date window (not description), so a
    // same-day, same-amount coincidence between two genuinely unrelated
    // transactions could get hidden by mistake; that's an accepted tradeoff
    // since a hide is reversible from Settings > Hidden imports, not a
    // delete.
    let duplicateCandidates = []
    if (addedForAlerts.length > 0) {
      const mapped = addedForAlerts.map(t => plaidTxnToRow(t, { userId, itemRowId: item.id }))
      duplicateCandidates = await findDuplicateCandidates(userId, mapped)
      if (duplicateCandidates.length > 0) {
        await setTransactionsHidden(
          userId,
          duplicateCandidates.map(c => c.uploadTransactionId),
          true,
          'duplicate_of_plaid'
        )
      }
    ...
    return { ok: true, ...totals, hiddenDuplicates: duplicateCandidates.length }
  } catch (error) {
    const code = plaidErrorCode(error)
