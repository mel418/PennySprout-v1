...
export function parseDate(t) {
  ...
  // ISO date-only (what the transactions table stores) must parse as LOCAL
  // time: new Date('2026-07-04') is UTC midnight, which is the previous day
  // in US timezones — every transaction would shift a day on the calendar.
  if (/^\d{4}-\d{2}-\d{2}$/.test(String(raw))) return fromKey(raw)
